...
limits = struct( ...
    'Temperature_C',            [0,    35  ], ...
    'Salinity_pss',             [30,   37  ], ...
    'Pressure_dbar',            [0,    1500], ...
    'Depth_m',                  [0,    1500], ...
    'Sigma_theta_kg_m3',        [20,   30  ], ...
...
for i = 1:numel(files)
    ...
    for p = 1:numel(params)
        ...
        flagType = cell(numel(allOut), 1);
        for k = 1:numel(allOut)
            r = ismember(allOut(k), rangeOut);
            s = ismember(allOut(k), statOut);
            if r && s,      flagType{k} = 'range+stat';
            elseif r,       flagType{k} = 'range';
            else,            flagType{k} = 'stat';
            end
        end
        ...
        rows = table( ...
            ...
            flagType, ...
            'VariableNames', {'File','Parameter','Row','Value','RangeLo','RangeHi','ColMean','ColStd','FlagType'});
